            ...
            with open('db.txt', 'r', encoding='UTF-8') as f:
                for line in f:
                    ...
                    if line and line.count(':') >= 2:
                        ...
                        media_files.append({'filename': filename, 'caption': caption, 'play_url': url_for('play_media', uuid_str=uid), 'download_url': url_for('download_media', uuid_str=uid)})
        except FileNotFoundError:
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            with open('db.txt', 'a', encoding='UTF-8') as f:
                f.write(f'{uuid_str}:{path.replace("\\", "/")}:{caption}\n')
...
def edit_profile():
    if 'username' in session:
        ...
        if request.method == 'POST':
            ...
            profile_picture = None  # define profile_picture here
            if 'profile_picture' in request.files:
                profile_picture = request.files['profile_picture']
            if profile_picture:  # now this should work
            ...
            with open('userbase.txt', 'w') as file:
                for uname, udata in users.items():
                    file.write(f"{uname},{udata['password']},{udata['profile_picture']},{udata['bio']}\n")
            flash('Profile updated successfully', 'uccess')
...
def profile(username):
    ...
    if username in users:
        user = users[username]
        user['media'] = []  # List to store user's media

        # Always use the default profile picture from static/profile_pictures
        user['profile_picture'] = 'default_profile_picture.png'  # or any other default image

        # Get user's media from db.txt
        try:
            with open('db.txt', 'r', encoding='UTF-8') as f:
                for line in f:
                    parts = line.strip().split(':')
